...
function AdminGroups() {
  ...
  const remove = useMutation({ mutationFn: (id: string) => removeGroup({ data: { id } }), onSuccess: () => { void queryClient.invalidateQueries({ queryKey: ["admin", "specialist-groups"] }); setDeleteTarget(null); toast.success("Ash group deleted."); }, onError: (error) => toast.error(error instanceof Error ? error.message : "The Ash group could not be deleted.") });
  ...
  return <div className="space-y-6">
    ...
    <DangerConfirmDialog
      ...
      title={`Delete ${deleteTarget?.name ?? "Ash group"}?`}
      warning="This permanently removes the group, roster and service setup."
      bullets={["This action cannot be undone.", "Groups with booking history cannot be deleted; move them to Draft instead.", "Your password or authenticator code is required."]}
      phrase="DELETE"
      confirmLabel="Delete group"
      destructive
      onConfirm={async () => { if (!deleteTarget) return; await remove.mutateAsync(deleteTarget.id); }}
    />
  </div>;
}
