...
export const Route = createFileRoute("/ashnight-control/groups")({
  head: () => ({ meta: [
    ...
    { name: "description", content: "Manage specialist teams, services, booking access and escrow payout allocations." },
    { property: "og:title", content: "Specialist Groups | Ashnight Admin" },
    { property: "og:description", content: "Manage specialist teams, services, booking access and escrow payout allocations." },
...
function AdminGroups() {
  ...
  const save = useMutation({ mutationFn: (value: Draft) => saveGroup({ data: value }), onSuccess: () => { void queryClient.invalidateQueries({ queryKey: ["admin", "specialist-groups"] }); setDraft(null); toast.success("Specialist group saved."); }, onError: (error) => toast.error(error instanceof Error ? error.message : "The group could not be saved.") });
  const status = useMutation({ mutationFn: (input: { id: string; active: boolean }) => changeStatus({ data: input }), onSuccess: () => void queryClient.invalidateQueries({ queryKey: ["admin", "specialist-groups"] }), onError: (error) => toast.error(error instanceof Error ? error.message : "Status could not be changed.") });
  ...
  return <div className="space-y-6">
    <header className="flex flex-wrap items-end justify-between gap-4"><div><p className="eyebrow text-primary">Operations</p><h1 className="mt-2 font-display text-2xl font-semibold sm:text-3xl">Specialist groups</h1><p className="mt-1 max-w-2xl text-sm text-muted-foreground">Build vetted crews, set client access, and freeze each member&apos;s escrow allocation when booked.</p></div><Button disabled={!canWrite} onClick={() => setDraft(emptyDraft())}><Plus className="size-4" /> Create group</Button></header>
    ...
    <Card className="overflow-hidden"><div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Group</TableHead><TableHead>Lead</TableHead><TableHead>Roster</TableHead><TableHead>Room</TableHead><TableHead>From</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Action</TableHead></TableRow></TableHeader><TableBody>
      {rows.map((group) => { const lead = group.specialist_group_members.find((member) => member.is_lead)?.profiles?.display_name ?? "Not assigned"; return <TableRow key={group.id}><TableCell><p className="font-medium">{group.name}</p><p className="text-xs text-muted-foreground">{group.specialist_group_services.length} services</p></TableCell><TableCell>{lead}</TableCell><TableCell><span className="inline-flex items-center gap-1.5"><Users className="size-4 text-muted-foreground" />{group.specialist_group_members.length}</span></TableCell><TableCell><TierBadge tier={group.room} /></TableCell><TableCell>{money(group.base_rate)}{group.pricing_model === "hourly" ? "/h" : ""}</TableCell><TableCell><Badge variant={group.active ? "success" : "secondary"}>{group.active ? "Active" : "Draft"}</Badge></TableCell><TableCell className="text-right"><div className="inline-flex gap-1"><Button variant="ghost" size="icon" aria-label={`Edit ${group.name}`} disabled={!canWrite} onClick={() => editGroup(group)}><Pencil className="size-4" /></Button><Button variant="outline" size="sm" disabled={!canWrite || status.isPending} onClick={() => status.mutate({ id: group.id, active: !group.active })}>{group.active ? "Pause" : "Activate"}</Button></div></TableCell></TableRow>; })}
      {!groups.isLoading && rows.length === 0 && <TableRow><TableCell colSpan={7} className="h-32 text-center text-muted-foreground">No specialist groups match this view.</TableCell></TableRow>}
...
function GroupEditor({ draft, setDraft, specialists, services, saving, onSave }: { draft: Draft | null; setDraft: (value: Draft | null) => void; specialists: Array<{ id: string; display_name: string }>; services: Array<{ id: string; name: string; active: boolean; base_rate: number }>; saving: boolean; onSave: () => void }) {
  ...
  return <Dialog open onOpenChange={(open) => !open && setDraft(null)}><DialogContent className="max-h-[90dvh] max-w-4xl overflow-y-auto"><DialogHeader><DialogTitle>{draft.id ? "Edit specialist group" : "Create specialist group"}</DialogTitle><DialogDescription>Define the roster, client-facing services and the exact escrow split.</DialogDescription></DialogHeader><div className="grid gap-5 md:grid-cols-2">
