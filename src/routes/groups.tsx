...
function GroupsPage() {
  ...
  const avatarItems = useMemo(() => (groups.data ?? []).flatMap((group) => group.specialist_group_members.flatMap((member) => member.profiles?.avatar_url ? [{ bucket: "avatars" as const, value: member.profiles.avatar_url }] : [])), [groups.data]);
  ...
  const request = useMutation({
    mutationFn: () => {
      ...
      return requestBooking({ data: { groupId: selected.id, serviceId, hours, scheduledFor: scheduledFor ? new Date(scheduledFor).toISOString() : null, notes, addons: [] } });
    },
    onSuccess: (result) => { toast.success("Request sent to the Ash group"); window.location.href = `/messages?thread=${encodeURIComponent(result.thread_id)}`; },
    onError: (error) => toast.error(error instanceof Error ? error.message : "The Ash group request could not be sent."),
  });
  ...
  <Dialog open={Boolean(selected)} onOpenChange={(openState) => !openState && setSelected(null)}><DialogContent className="max-h-[90dvh] overflow-y-auto"><DialogHeader><DialogTitle>{selected?.name}</DialogTitle><DialogDescription>{selected?.description}</DialogDescription></DialogHeader>{selected && <div className="space-y-4"><div className="flex flex-wrap gap-2">{selected.specialist_group_members.map((member) => { const name = member.profiles?.display_name ?? "Specialist"; const storedAvatar = member.profiles?.avatar_url; const avatarUrl = storedAvatar ? memberMedia?.[storedAvatar] : undefined; return <Badge key={member.id} variant="secondary" className="h-auto gap-2 py-1 pr-3 pl-1"><Avatar className="size-7 border border-border"><AvatarImage src={avatarUrl} alt={name} /><AvatarFallback className="text-[10px]">{initials(name)}</AvatarFallback></Avatar><span>{name}{member.is_lead ? " · Lead" : ""}</span></Badge>; })}</div><div className="space-y-2"><Label>Service</Label><Select value={serviceId} onValueChange={(value) => { setServiceId(value); const match = selected.specialist_group_services.find((item) => item.service_id === value); if (match) setHours(match.minimum_hours); }}><SelectTrigger><SelectValue placeholder="Choose a service" /></SelectTrigger><SelectContent>{selected.specialist_group_services.map((item) => <SelectItem key={item.id} value={item.service_id}>{item.services?.name ?? "Group service"} · {money(item.rate)}/h</SelectItem>)}</SelectContent></Select></div><div className="grid grid-cols-2 gap-3"><div className="space-y-2"><Label htmlFor="group-hours">Hours</Label><Input id="group-hours" type="number" min={minimumHours} max={48} step="0.5" value={hours} onChange={(event) => setHours(event.target.value === "" ? 0 : Number(event.target.value))} /><p className="text-xs text-muted-foreground">Minimum {minimumHours} hour{minimumHours === 1 ? "" : "s"}</p></div><div className="space-y-2"><Label htmlFor="group-time">Preferred time</Label><Input id="group-time" type="datetime-local" min={new Date().toISOString().slice(0, 16)} value={scheduledFor} onChange={(event) => setScheduledFor(event.target.value)} /></div></div><div className="space-y-2"><Label htmlFor="group-notes">Visit notes</Label><Textarea id="group-notes" value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Access, priorities, or special instructions" /></div><div className="flex items-start gap-2 rounded-md border p-3 text-xs text-muted-foreground"><ShieldCheck className="mt-0.5 size-4 shrink-0 text-primary" /><p>No payment is taken now. The crew confirms availability in the shared chat first; payment opens only when everyone accepts.</p></div></div>}<DialogFooter><Button variant="outline" onClick={() => setSelected(null)}>Cancel</Button><Button disabled={request.isPending || !serviceId || !validHours} onClick={() => request.mutate()} title={!validHours ? `This service requires at least ${minimumHours} hours` : undefined}>{request.isPending ? "Creating shared chat…" : "Request this Ash group"}</Button></DialogFooter></DialogContent></Dialog>
  </div>;
}