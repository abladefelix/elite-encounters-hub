...
export const Route = createFileRoute("/groups")({
  head: () => ({ meta: [
    { title: "Book a Specialist Team | Ashnight" },
    { name: "description", content: "Book an admin-curated team of vetted cleaning specialists with one payment and protected member-by-member escrow." },
    { property: "og:title", content: "Book a Specialist Team | Ashnight" },
    { property: "og:description", content: "Vetted cleaning teams, one checkout, and protected member-by-member escrow." },
...
function GroupsPage() {
  ...
  return <div className="min-h-screen pb-20 md:pb-0"><SiteHeader /><main className="mx-auto w-full max-w-6xl px-5 py-10">
    ...
    : <><header className="max-w-3xl"><p className="eyebrow text-primary">Collective care</p><h1 className="mt-2 font-display text-3xl font-semibold sm:text-4xl">A vetted team for work that needs more hands</h1><p className="mt-3 text-muted-foreground">One booking coordinates the full crew. Each specialist&apos;s share is fixed before payment and protected separately in escrow.</p></header>
      ...
      {!groups.isLoading && (groups.data?.length ?? 0) === 0 && <div className="mt-10 rounded-md border border-dashed p-10 text-center text-sm text-muted-foreground">No specialist teams are open to your room right now.</div>}
    </>}
  </main><SiteFooter />
