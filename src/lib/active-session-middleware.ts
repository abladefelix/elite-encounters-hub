...
export const requireActiveSession = createMiddleware({ type: "function" })
  ...
  .server(async ({ next, context }) => {
    ...
    const deviceId = request.headers.get("x-ashnight-device-id") ?? undefined;
    const { registerSession, validateSession } = await import("./session-management.server");
    let result = await validateSession(context.userId, token, authSessionId, deviceId);
    if (!result.valid && result.reason === "This session is no longer registered.") {
      await registerSession({
        userId: context.userId,
        accessToken: token,
        ...(authSessionId ? { authSessionId } : {}),
        deviceId: deviceId ?? `existing-${context.userId}`,
        deviceName: "Existing signed-in device",
        userAgent: request.headers.get("user-agent") ?? "",
        ip: request.headers.get("cf-connecting-ip") ?? request.headers.get("x-forwarded-for") ?? "",
      });
      result = await validateSession(context.userId, token, authSessionId, deviceId);
    }
