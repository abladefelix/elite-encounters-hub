...
import { CheckCircle2, CircleAlert, ShieldCheck, Users, XCircle } from "lucide-react";
...
export function GroupBookingDialog({ group, onClose }: { group: BookableGroup | null; onClose: () => void }) {
  ...
  const availableMembers = group?.specialist_group_members.filter((member) => member.profiles?.available) ?? [];
  const unavailableMembers = group?.specialist_group_members.filter((member) => !member.profiles?.available) ?? [];
  useEffect(() => {
    if (!group) return;
  ...
  const close = () => { setServiceId(""); setHours(2); setScheduledFor(""); setNotes(""); setConfirmingRoster(false); onClose(); };

  return <><Dialog open={Boolean(group)} onOpenChange={(open) => !open && close()}><DialogContent className="max-h-[90dvh] overflow-y-auto sm:max-w-xl"><DialogHeader><div className="flex items-center gap-2"><Badge variant="secondary"><Users className="mr-1 size-3" /> Ash group</Badge></div><DialogTitle>{group?.name}</DialogTitle><DialogDescription>{group?.description}</DialogDescription></DialogHeader>{group && <div className="space-y-5"><section><p className="text-xs font-semibold uppercase text-muted-foreground">Proposed crew</p><div className="mt-2 space-y-2">{group.specialist_group_members.map((member) => { const name = member.profiles?.display_name ?? "Specialist"; const storedAvatar = member.profiles?.avatar_url; const avatarUrl = storedAvatar ? memberMedia?.[storedAvatar] : undefined; const isAvailable = Boolean(member.profiles?.available); return <div key={member.id} className="flex items-center gap-3 rounded-md border border-border/70 p-2"><Avatar className="size-9 border border-border"><AvatarImage src={avatarUrl} alt={name} /><AvatarFallback className="text-xs">{initials(name)}</AvatarFallback></Avatar><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{name}</p><p className="truncate text-xs text-muted-foreground">{member.is_lead ? "Lead contact" : member.role_label}</p></div><Badge variant={isAvailable ? "success" : "secondary"} className="shrink-0">{isAvailable ? <CheckCircle2 className="mr-1 size-3" /> : <XCircle className="mr-1 size-3" />}{isAvailable ? "Available" : "Unavailable"}</Badge></div>; })}</div><p className="mt-2 text-xs text-muted-foreground">Availability is shown before the request. Every member must still confirm the selected date and time in the shared chat before payment.</p></section><div className="space-y-2"><Label>Service</Label><Select value={activeServiceId} onValueChange={(value) => { setServiceId(value); const match = group.specialist_group_services.find((item) => item.service_id === value); if (match) setHours(match.minimum_hours); }}><SelectTrigger><SelectValue placeholder="Choose a service" /></SelectTrigger><SelectContent>{group.specialist_group_services.map((item) => <SelectItem key={item.id} value={item.service_id}>{item.services?.name ?? "Ash group service"} · {money(item.rate)}/h</SelectItem>)}</SelectContent></Select></div><div className="grid grid-cols-2 gap-3"><div className="space-y-2"><Label htmlFor="team-hours">Hours</Label><Input id="team-hours" type="number" min={minimumHours} max={48} step="0.5" value={hours} onChange={(event) => setHours(event.target.value === "" ? 0 : Number(event.target.value))} /><p className="text-xs text-muted-foreground">Minimum {minimumHours} hour{minimumHours === 1 ? "" : "s"}</p></div><div className="space-y-2"><Label htmlFor="team-time">Preferred time</Label><Input id="team-time" type="datetime-local" min={new Date().toISOString().slice(0, 16)} value={scheduledFor} onChange={(event) => setScheduledFor(event.target.value)} /></div></div><div className="space-y-2"><Label htmlFor="team-notes">Service instructions</Label><Textarea id="team-notes" value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Access, priorities, and instructions for the whole crew" /></div><div className="flex items-start gap-2 rounded-md border p-3 text-xs text-muted-foreground"><ShieldCheck className="mt-0.5 size-4 shrink-0 text-primary" /><p>No payment is taken now. The roster is checked first; after everyone confirms, the agreed total is locked and Paystack becomes available in chat.</p></div></div>}<DialogFooter><Button variant="outline" onClick={close}>Cancel</Button><Button disabled={request.isPending || !activeServiceId || !validHours} onClick={() => setConfirmingRoster(true)} title={!validHours ? `This service requires at least ${minimumHours} hours` : undefined}>{request.isPending ? "Creating shared chat…" : "Choose this Ash group"}</Button></DialogFooter></DialogContent></Dialog><AlertDialog open={confirmingRoster} onOpenChange={setConfirmingRoster}><AlertDialogContent className="max-h-[85dvh] overflow-y-auto"><AlertDialogHeader><AlertDialogTitle>Confirm the current crew availability</AlertDialogTitle><AlertDialogDescription>Review who is currently available before starting the shared chat. No payment will be taken at this stage.</AlertDialogDescription></AlertDialogHeader><div className="grid gap-3 sm:grid-cols-2"><div className="rounded-md border border-border p-3"><p className="flex items-center gap-2 text-sm font-semibold"><CheckCircle2 className="size-4 text-success" />Available ({availableMembers.length})</p><ul className="mt-2 space-y-1 text-sm text-muted-foreground">{availableMembers.length ? availableMembers.map((member) => <li key={member.id}>{member.profiles?.display_name ?? "Specialist"}</li>) : <li>None currently marked available</li>}</ul></div><div className="rounded-md border border-border p-3"><p className="flex items-center gap-2 text-sm font-semibold"><CircleAlert className="size-4 text-destructive" />Unavailable ({unavailableMembers.length})</p><ul className="mt-2 space-y-1 text-sm text-muted-foreground">{unavailableMembers.length ? unavailableMembers.map((member) => <li key={member.id}>{member.profiles?.display_name ?? "Specialist"}</li>) : <li>Everyone is currently available</li>}</ul></div></div>{unavailableMembers.length > 0 && <p className="rounded-md border border-destructive/30 bg-destructive/10 p-3 text-sm text-destructive">You can continue, but the unavailable members may decline. If anyone declines, the request closes automatically and no payment is taken.</p>}<AlertDialogFooter><AlertDialogCancel>Go back</AlertDialogCancel><AlertDialogAction onClick={() => request.mutate()}>Accept &amp; start chat</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog></>;
}