...
export function GroupSpecialistTile({ group, onSelect }: { group: GroupSpecialistTileData; onSelect: () => void }) {
  ...
  return <Button variant="ghost" className="group h-auto w-full flex-col items-stretch overflow-hidden border border-border/70 bg-surface-strong p-0 text-left hover:bg-surface-strong" onClick={onSelect}>
    <div className="relative aspect-[4/5] w-full overflow-hidden">
      {imageUrl ? <img src={imageUrl} alt={`${group.name}, specialist team`} className="size-full object-cover transition-transform duration-300 group-hover:scale-105" loading="lazy" /> : <div className="flex size-full items-center justify-center bg-panel font-display text-2xl text-muted-foreground">{initials(group.name)}</div>}
