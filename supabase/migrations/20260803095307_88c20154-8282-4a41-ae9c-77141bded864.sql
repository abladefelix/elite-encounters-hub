CREATE OR REPLACE FUNCTION private.can_review(_rated uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
CREATE POLICY "Clients review after a completed job"
  ...
  WITH CHECK (rater_id = auth.uid() AND private.can_review(rated_id));

DROP FUNCTION IF EXISTS public.can_review(uuid);