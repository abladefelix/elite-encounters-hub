DROP FUNCTION public.create_group_booking_snapshot(uuid, uuid, numeric, timestamptz, text, text[], text);
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = caller AND role = 'client') THEN RAISE EXCEPTION 'Only clients can book specialist groups'; END IF;
  SELECT room INTO caller_room FROM public.profiles WHERE id = caller AND account_status = 'active' AND suspended = false AND vetting = 'approved';
  ...
  IF EXISTS (SELECT 1 FROM public.specialist_group_members gm LEFT JOIN public.profiles p ON p.id = gm.specialist_id LEFT JOIN public.user_roles ur ON ur.user_id = gm.specialist_id AND ur.role = 'specialist' WHERE gm.group_id = grp.id AND gm.active AND (p.id IS NULL OR p.vetting <> 'approved' OR p.account_status <> 'active' OR p.suspended OR ur.user_id IS NULL)) THEN RAISE EXCEPTION 'This group roster contains an unavailable specialist'; END IF;
  SELECT count(*), max(specialist_id) FILTER (WHERE is_lead) INTO member_count, lead_id FROM public.specialist_group_members WHERE group_id = grp.id AND active;
  ...
  INSERT INTO public.group_bookings (group_id, client_id, service_id, service_name, scheduled_for, hours, addons, notes, subtotal, platform_fee_pct, platform_fee, total, status, paystack_reference, allocation_locked) VALUES (grp.id, caller, _service_id, svc.name, _scheduled_for, _hours, _addons, left(coalesce(_notes, ''), 1200), subtotal_value, fee_pct, fee_value, total_value, 'requested', _paystack_reference, true) RETURNING id INTO booking_id;
  INSERT INTO public.threads (client_id, specialist_id, room, is_group, group_booking_id, last_message) VALUES (caller, lead_id, grp.room, true, booking_id, 'Group booking created — payment awaiting confirmation.') RETURNING id INTO conversation_id;
  INSERT INTO public.thread_participants (thread_id, user_id, participant_role, can_manage) VALUES (conversation_id, caller, 'client', false);
  FOR member IN SELECT * FROM public.specialist_group_members WHERE group_id = grp.id AND active ORDER BY is_lead DESC, created_at, id LOOP
    ...
    INSERT INTO public.group_booking_members (group_booking_id, specialist_id, role_label, is_lead, share_pct, allocated_amount, platform_fee, payout_amount, status) VALUES (booking_id, member.specialist_id, member.role_label, member.is_lead, member.share_pct, leg_subtotal + leg_fee, leg_fee, leg_subtotal, 'assigned') RETURNING id INTO booking_member_id;
    INSERT INTO public.escrow_entries (kind, thread_id, group_booking_id, group_booking_member_id, client_id, specialist_id, label, amount, platform_fee, payout_amount, hold_hours, state, paystack_reference) VALUES ('booking', conversation_id, booking_id, booking_member_id, caller, member.specialist_id, grp.name || ' · ' || svc.name || ' · ' || _hours || 'h', leg_subtotal + leg_fee, leg_fee, leg_subtotal, hold_window, 'pending', _paystack_reference);
    INSERT INTO public.thread_participants (thread_id, user_id, participant_role, can_manage) VALUES (conversation_id, member.specialist_id, CASE WHEN member.is_lead THEN 'lead' ELSE 'specialist' END, member.is_lead) ON CONFLICT (thread_id, user_id) DO UPDATE SET participant_role = EXCLUDED.participant_role, can_manage = EXCLUDED.can_manage;
  END LOOP;
...
END;
$$;
REVOKE ALL ON FUNCTION public.create_group_booking_snapshot(uuid, uuid, numeric, uuid, timestamptz, text, text[], text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.create_group_booking_snapshot(uuid, uuid, numeric, uuid, timestamptz, text, text[], text) TO service_role;

DROP FUNCTION public.cancel_unpaid_group_booking(uuid);
...
BEGIN
  ...
  SELECT t.id INTO conversation_id FROM public.group_bookings gb LEFT JOIN public.threads t ON t.group_booking_id = gb.id WHERE gb.id = _group_booking_id AND gb.client_id = _requesting_user AND gb.paid_at IS NULL AND gb.status = 'requested' FOR UPDATE OF gb;
  ...
  IF conversation_id IS NOT NULL THEN DELETE FROM public.messages WHERE thread_id = conversation_id; DELETE FROM public.thread_participants WHERE thread_id = conversation_id; DELETE FROM public.threads WHERE id = conversation_id; END IF;
...
END;
$$;
REVOKE ALL ON FUNCTION public.cancel_unpaid_group_booking(uuid, uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.cancel_unpaid_group_booking(uuid, uuid) TO service_role;